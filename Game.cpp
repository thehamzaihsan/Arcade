...
//		while (windowp->pollEvent(*eventp)) {
...
void Game::Update()
{
	...
	int i = eventp->mouseButton.x / 40;
	int j = eventp->mouseButton.y / 40;

	if (!isExploded && !isWin)
	{
		if (eventp->type == sf::Event::MouseButtonPressed)
		{
			if (i > 0 && j > 0 && i < WINDOW_HEIGTH / TILE_WIDTH - 1 && j < WINDOW_WIDTH / TILE_WIDTH - 1)
			{
				if (eventp->key.code == sf::Mouse::Left)
				{
					if (FirstMove == true)
					{
				...
				if (eventp->key.code == sf::Mouse::Right)
				{

					tile_array[i][j].setIsFlagged(true);
				}
				if (eventp->key.code == sf::Mouse::Middle)
				{
	...
	for (int i = 0; i < WINDOW_HEIGTH / TILE_WIDTH; i++)
	{
		for (int j = 0; j < WINDOW_WIDTH / TILE_WIDTH; j++)
		{
			...
			if (tile_array[i][j].gethidden_state() == true)
			{
				...
				{
					windowp->draw(tile_array[i][j].getInnerText());
				}
			}

			windowp->draw(tile_array[i][j].getSprite());
		}
	}
